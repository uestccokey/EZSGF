...
public class SgfParser {
    ...
    private Stack<SgfNode> mTreeStack = new Stack<>();

    public SgfParser(String game) {
        mOriginalGame = game;
    }

    public SgfGame parse() {
//        SgfGame game = new SgfGame(mOriginalGame);
        SgfGame game = new SgfGame();

        // the root node
        SgfNode parentNode = null;
        ...
        for (int i = 0; i < mOriginalGame.length(); i++) {
            ...
            if (';' == chr && (i == 0 || mOriginalGame.charAt(i - 1) != '\\')) {
                ...
                SgfNode node = parseToken(nodeContents, parentNode, game);
                if (node.isMove()) {
    ...
    private SgfNode parseToken(String token, final SgfNode parentNode, SgfGame game) {
        SgfNode rtrnNode = new SgfNode(parentNode);
        // replace delimiters
        token = SgfParser.prepareToken("'" + token + "'");
        ...
        while (m.find()) {
            ...
            value = SgfParser.normaliseToken(value);
            ...
            if ("AB".equals(key) || "AW".equals(key) || "AE".equals(key)) {
                ...
                if (parentNode == null) {
                    game.addProperty(key, TextUtils.join(",", list));
                } else {
                    rtrnNode.addProperty(key, TextUtils.join(",", list));
                }
