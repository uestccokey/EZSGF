...
/**
 * Computes the visual depth for the game nodes.
 * <p>
 * The visual depth designates as how deep in the tree should the line be shown if a GUI is being used.
 * I should move this helper to sgf4j-gui project but haven't done it yet.
 */
class VisualDepthHelper {

//    private List<List<Integer>> mDepthMatrix;
     ...
     *
     * @param variationDepth depth of the variation for the first level child of the line of play.
     */
    void calculateVisualDepth(SgfNode lastNode, int variationDepth) {
        ...
        SgfNode activeNode = lastNode;
        do {
            if (activeNode.hasChildren()) {
                for (SgfNode gameNode : activeNode.getChildren()) {
                    ...
                    calculateVisualDepthFor(gameNode, depthMatrix, 1, variationDepth);
                }
            }
        } while ((activeNode = activeNode.getPrevNode()) != null);
//        this.mDepthMatrix = depthMatrix;
    }

    private void calculateVisualDepthFor(SgfNode node, List<List<Integer>> depthMatrix, int minDepth, int variationDepth) {
        int depth = findVisualDepthForNode(node, depthMatrix, minDepth, variationDepth);

        SgfNode activeNode = setVisualDepthForLine(node, depth);
        do {
            if (activeNode.hasChildren()) {
                for (SgfNode gameNode : activeNode.getChildren()) {
                    calculateVisualDepthFor(gameNode, depthMatrix, depth + 1, variationDepth);
                }
            }
    ...
    private void initializeMainLine(SgfNode lastNode, List<List<Integer>> depthMatrix) {
    ...
    private SgfNode setVisualDepthForLine(SgfNode child, int depth) {
        SgfNode node = child;
        SgfNode rtrn;
        do {
    ...
    private int findVisualDepthForNode(SgfNode node, List<List<Integer>> depthMatrix, int minDepth, int variationDepth) {
     ...
     */
    private void bookForLineOfPlay(SgfNode node, int length, List<List<Integer>> depthMatrix, int listIndex, int variationDepth) {
     ...
     */
    private boolean isAvailableForLineOfPlay(SgfNode node, int length, List<List<Integer>> depthMatrix, int listIndex, int variationDepth) {
...
//        for (List<Integer> list : mDepthMatrix) {
...
//    }
     ...
     */
    private int findLengthOfLine(final SgfNode node) {
        SgfNode tmpNode = node;
        int i = 0;
        do {
            i++;
